...
void ParseInfoPartOfDay(
    DayPartWeatherInfo& data, IndexPartsOfDay part_of_day, size_t& iteration,
  ...
  SetWeatherInfoForPartDay(data, iteration, static_cast<uint8_t>(part_of_day), city_name,
                           city_forecast);
}
...
void ParseRequest(
  ...
  for (size_t i = 0; i < counter; ++i, time_iter += kCountHourInDay) {
    ...
    ParseInfoPartOfDay(data, IndexPartsOfDay::Night, i, info, city.name, city_forecast);
    data = DayPartWeatherInfo();
    data.start_hour = kStartHourMorning;
    data.end_hour = kEndHourMorning;
    ParseInfoPartOfDay(data, IndexPartsOfDay::Morning, i, info, city.name, city_forecast);
    data = DayPartWeatherInfo();
    data.start_hour = kStartHourDay;
    data.end_hour = kEndHourDay;
    ParseInfoPartOfDay(data, IndexPartsOfDay::Day, i, info, city.name, city_forecast);
    data = DayPartWeatherInfo();
    data.start_hour = kStartHourEvening;
    data.end_hour = kEndHourEvening;
    ParseInfoPartOfDay(data, IndexPartsOfDay::Evening, i, info, city.name, city_forecast);
...
std::map<std::string, std::vector<AllDayWetherInfo>> GetInfoForForecast(
  ...
  for (auto city : data_from_config["cities"]) {
    ...
    SetCoords(geographical_info[city]);
...
std::string RoundDoubleToString(std::string number, uint16_t count) {
  std::string result;
...
Element DesignPartOfDay(AllDayWetherInfo& info, const uint8_t& index,
                        json& data) {
...
Element DesignOfDay(AllDayWetherInfo& info, json& data) {
  Element design = hbox(DesignPartOfDay(info, kIndexNight, data),
                        DesignPartOfDay(info, kIndexMorning, data),
                        DesignPartOfDay(info, kIndexDay, data),
                        DesignPartOfDay(info, kIndexEvening, data));
...
void DrawForecast(const std::string& directory) {
  ...
  auto renderer = Renderer([&] {
    ...
    for (size_t i = 0; i < counter; ++i) {
      forecasts.push_back(
          DesignOfDay(city_forecast[city][i], data_from_config));
    }
